'read_prep_rawfiles_with_analog_folder_2021.R
...
if (exists("ClassyFire_hits")) {
  df.featureID <-
   ...
   df.featureID <-
    cbind(
      df.featureID,
      feature_info$superclass_Analog,
      feature_info$class_Analog,
      feature_info$subclass_Analog)
      }
...
tmp <-as.data.frame(matrix(ncol = ncol(df.featureID), nrow = 1)) #make dataframe to store colnames in
